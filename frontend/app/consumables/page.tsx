                    ...
                    <div className="rounded-2xl w-[92%] max-w-lg shadow-2xl overflow-hidden flex flex-col" style={{ background: "var(--t-modal-bg)", border: "1px solid var(--t-modal-border)", maxHeight: "85vh" }} onClick={(e) => e.stopPropagation()}>
                        ...
                        <div className="flex-1 overflow-y-auto p-5 space-y-4">
                            ...
                            <div className="grid grid-cols-3 gap-3">
                                ...
                                <div><label className="text-xs font-semibold mb-1.5 block" style={{ color: "var(--t-text-secondary)" }}>หน่วยนับ</label><select value={createForm.unit} onChange={(e) => setCreateForm({ ...createForm, unit: e.target.value })} className="w-full rounded-lg px-3 py-2.5 text-sm focus:outline-none cursor-pointer" style={{ background: "var(--t-input-bg)", border: "1px solid var(--t-input-border)", color: "var(--t-input-text)" }}>{["ชิ้น", "ขวด", "ม้วน", "หลอด", "แผ่น", "กระป๋อง", "ถุง", "กล่อง", "แกลลอน", "ตัว", "คู่", "ชุด", "ดวง"].map(u => <option key={u} value={u}>{u}</option>)}</select></div>
                            </div>
                        </div>
                        {/* Footer */}
                        <div className="px-5 py-4 flex gap-3 shrink-0" style={{ borderTop: "1px solid var(--t-border-subtle)" }}>
                            ...
                            <button onClick={async () => { if (!createForm.code || !createForm.name) { setCreateError("กรุณากรอกรหัสและชื่อ"); return; } setCreateSaving(true); setCreateError(""); try { await createPart({ ...createForm, quantity: Number(createForm.quantity), minStock: Number(createForm.minStock), categoryId: consumableId }); setShowCreate(false); toast.success("สร้างวัสดุเรียบร้อย"); fetchParts(); } catch (err: any) { setCreateError(err.message || "เกิดข้อผิดพลาด"); } finally { setCreateSaving(false); } }} disabled={createSaving} className="flex-1 text-white font-bold rounded-xl py-2.5 text-sm cursor-pointer disabled:opacity-30 disabled:cursor-not-allowed transition-all shadow-lg hover:-translate-y-0.5" style={{ background: "linear-gradient(135deg, #f59e0b, #d97706)", boxShadow: "0 4px 14px rgba(245,158,11,0.35)" }}>{createSaving ? "กำลังบันทึก..." : "สร้างวัสดุ"}</button>
                        </div>
                    </div>
                </div>
            )}
                    ...
                    <div className="rounded-2xl w-[92%] max-w-lg shadow-2xl overflow-hidden flex flex-col" style={{ background: "var(--t-modal-bg)", border: "1px solid var(--t-modal-border)", maxHeight: "85vh" }} onClick={(e) => e.stopPropagation()}>
                        ...
                        <div className="flex-1 overflow-y-auto p-5 space-y-4">
                            ...
                            <div className="grid grid-cols-2 gap-3">
                                ...
                                <div><label className="text-xs font-semibold mb-1.5 block" style={{ color: "var(--t-text-secondary)" }}>หน่วยนับ</label><select value={editPartForm.unit} onChange={(e) => setEditPartForm({ ...editPartForm, unit: e.target.value })} className="w-full rounded-lg px-3 py-2.5 text-sm focus:outline-none cursor-pointer" style={{ background: "var(--t-input-bg)", border: "1px solid var(--t-input-border)", color: "var(--t-input-text)" }}>{["ชิ้น", "ขวด", "ม้วน", "หลอด", "แผ่น", "กระป๋อง", "ถุง", "กล่อง", "แกลลอน", "ตัว", "คู่", "ชุด", "ดวง"].map(u => <option key={u} value={u}>{u}</option>)}</select></div>
                            </div>
                        </div>
                        {/* Footer */}
                        <div className="px-5 py-4 flex gap-3 shrink-0" style={{ borderTop: "1px solid var(--t-border-subtle)" }}>
