                            ...
                            <div key={p.id} className="rounded-xl p-4 transition-all cursor-pointer" style={{ background: "var(--t-card)", border: `1px solid ${isLow ? "rgba(239,68,68,0.25)" : "var(--t-border-subtle)"}` }} onMouseEnter={(e) => e.currentTarget.style.boxShadow = "0 2px 8px rgba(0,0,0,0.06)"} onMouseLeave={(e) => e.currentTarget.style.boxShadow = "none"} onClick={() => setBarcodePart(p)}>
                                <div className="flex items-center justify-between gap-4 flex-wrap">
                                    ...
                                    <div className="flex items-center gap-1.5 shrink-0 w-full sm:w-auto mt-2 sm:mt-0 pt-3 sm:pt-0 justify-end sm:border-t-0" style={{ borderTop: "1px dashed var(--t-border-subtle)" }}>
                                        ...
                                        {isAdmin && <button onClick={(e) => { e.stopPropagation(); setEditingPart(p); setEditPartForm({ code: p.code, name: p.name, description: p.description || "", brand: p.brand || "", specification: p.specification || "แม่สี", unit: p.unit, minStock: p.minStock }); setEditPartError(""); }} className="p-2 rounded-lg cursor-pointer flex-1 sm:flex-none flex items-center justify-center" style={{ background: "rgba(139,92,246,0.1)", color: "#8B5CF6" }} title="แก้ไข">
