...
import { Package, Search, Filter, TrendingDown, CheckCircle2, ScanBarcode, ArrowDownToLine, ArrowUpFromLine, Minus, Plus, X, AlertCircle, ChevronLeft, Car, PackagePlus } from "lucide-react";
...
export default function ShopPage() {
            ...
            <div className="p-6 lg:p-8">
                ...
                <div className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-5 gap-4">
                    {brands.map((brand) => (
                        <button
                            key={brand.id}
                            onClick={() => { setSelectedBrand(brand); setPage(1); setSearch(""); setLowStockOnly(false); router.push(`/shop?brand=${brand.id}`); }}
                ...
                {showAddBrand && (
                    <div className="fixed inset-0 z-50 flex items-center justify-center" style={{ background: "var(--t-modal-overlay)", animation: "fadeIn 150ms ease" }} onClick={() => setShowAddBrand(false)}>
                        <div className="rounded-2xl p-6 w-[90%] max-w-sm shadow-2xl" style={{ background: "var(--t-modal-bg)", border: "1px solid var(--t-modal-border)", animation: "slideUp 200ms ease" }} onClick={(e) => e.stopPropagation()}>
                            ...
                            <input value={newBrandName} onChange={(e) => setNewBrandName(e.target.value)} onKeyDown={async (e) => { if (e.key === "Enter" && newBrandName.trim()) { try { await createCategory({ name: newBrandName.trim(), parentId: shopRoot?.id }); setShowAddBrand(false); const c = await getCategories(); setAllCategories(c); } catch (err: any) { alert(err.message); } } }} placeholder="ชื่อยี่ห้อรถ (เช่น Nissan)" className="w-full rounded-lg px-3 py-2.5 text-sm mb-4 focus:outline-none focus:ring-1 focus:ring-emerald-500/30" style={{ background: "var(--t-input-bg)", border: "1px solid var(--t-input-border)", color: "var(--t-input-text)" }} autoFocus />
                            <div className="flex gap-3">
                                ...
                                <button onClick={async () => { if (!newBrandName.trim()) return; try { await createCategory({ name: newBrandName.trim(), parentId: shopRoot?.id }); setShowAddBrand(false); const c = await getCategories(); setAllCategories(c); } catch (err: any) { alert(err.message); } }} className="flex-1 bg-emerald-500 hover:bg-emerald-400 text-white font-semibold rounded-lg py-2.5 text-sm cursor-pointer">เพิ่ม</button>
                            </div>
                        </div>
                    </div>
                )}
    ...
    // ─── Parts List View (after selecting a brand) ──────────────
    return (
        <div className="p-6 lg:p-8">
            <div className="mb-8">
                <button
                    onClick={() => { setSelectedBrand(null); setParts([]); router.push("/shop"); }}
                ...
                <h1 className="text-xl font-bold" style={{ color: "var(--t-text)" }}>
                    อะไหล่หน้าร้าน — {selectedBrand.name}
                </h1>
